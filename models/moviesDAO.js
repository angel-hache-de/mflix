...
class MoviesDAO {
   ...
   * @param {string} moviesPerPage - Text to search in the title/description of movie
  ...
  static async facetedSearch({
    ...
    const textMatchStage = { $match: { $text: { $search: text } } };
    const matchStage = { $match: filters };

    let sortStage;
    if (textLength > 0)
      sortStage = { $sort: { score: { $meta: "textScore" } } };
    else sortStage = { $sort: { "tomatoes.viewer.numReviews": -1 } };
    let countingPipeline = [{ $count: "count" }];
    if (filtersLength > 0) countingPipeline.unshift(matchStage);
    if (textLength > 0) countingPipeline.unshift(textMatchStage);
    ...
    */
    ...
    if (filtersLength > 0) queryPipeline.unshift(matchStage);
    if (textLength > 0) queryPipeline.unshift(textMatchStage);

    try {
      const results = await (await movies.aggregate(queryPipeline)).toArray();
      const counting = await (
        await movies.aggregate(countingPipeline)
      ).toArray();

      return {
        ...results[0],
        count: counting[0].count,
      };
    } catch (e) {
      console.log(e);
  ...
  static async getMovies({
    ...
    if (filters) {
      ...
      if ("cast" in filters) {
        const queryP = this.castSearchQuery(filters["cast"]);
        if (Object.keys(queryParams).length)
          queryParams.query = { ...queryParams.query, ...queryP.query };
        else queryParams = queryP;
      }

      if ("genre" in filters) {
        const queryP = this.genreSearchQuery(filters["genre"]);
        if (Object.keys(queryParams).length)
          queryParams.query = { ...queryParams.query, ...queryP.query };
        else queryParams = queryP;
      }
    ...
    try {
      ...
      */
      ...
      if (
        ...
        e
          .toString()
          .startsWith("Error: Argument passed in must be a single String")
      ) {
        return null;
      }
